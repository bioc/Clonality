indiv.test <-
function(ss1,ss2,func,gainthres,lossthres,Nsim=100,sbdry=sbdry)
{
...
sseg1<-segment1(CNA(x1,rep(1,length(x1)),ss1$data$maploc),sbdry=sbdry)
sseg2<-segment1(CNA(x2,rep(1,length(x2)),ss2$data$maploc),sbdry=sbdry)
...
sseg1$output[sseg1$output$seg.mean>gainthres[1],7]<-"Gain"
sseg1$output[sseg1$output$seg.mean<lossthres[1],7]<-"Loss"

sseg2$output[sseg2$output$seg.mean>gainthres[2],7]<-"Gain"
sseg2$output[sseg2$output$seg.mean<lossthres[2],7]<-"Loss"

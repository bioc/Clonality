calc.freq <-
function(pfreq,refdata, classall,nmad,sbdry=NULL)
 { 
chrlist<-rownames(classall)
if (!is.null(pfreq)) 
{
if (ncol(pfreq)!=4) 
  stop("pfreq should have 4 columns: chromosome arm name, frequenciy of gain, 
  loss, normal. Please fix it.") 

if (!is.character(pfreq[,1]) | !all(nchar(pfreq[,1]==6)) | 
! all(substr(pfreq[,1],1,3) %in% c("Chr","chr")) | 
! all(substr(pfreq[,1],6,6) %in% c("p","q")))
 stop("Chromosome arm in the first column of pfreq should have
  format 'chr' + two digits + 'p or q', like in 'chr01p','Chr11q' etc")
...
refseg1<-segment1(refdata[refdata$chrom %in% chrlist,],sbdry=sbdry)

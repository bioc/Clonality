...
if (!all(is.numeric(chrom)))
      stop("Chromosome is not numeric, should be a number between 1 and 22") 
if (any(chrom>22)) warning("Chromosomes 23(X) and 24(Y) are discarded\n")
maploc<-maploc[chrom<=22 ]
chrom<-chrom[chrom<=22 ]
kb=1
if (mean(maploc[chrom==1]<centro[1],na.rm=TRUE) < 0.01) 
warning("There are too few makres in one of the arms. The maplocs should be in Kb") 
if (mean(maploc[chrom==1]>centro[1],na.rm=TRUE) < 0.01) 
warning("There are too few makres in one of the arms. The maplocs should be in Kb") 

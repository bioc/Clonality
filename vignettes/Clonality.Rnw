...
Marginal frequencies of mutations are assumed to be known and used as fixed quantities in the likelihood model. They can be estimated using function {\it get.mutation.frequencies} using either the built-in mutational data from TCGA pancancer study, or reference mutational data, or combination of the two. Currently, the built-in mutational frequencies datset 'freqdata' contains 3 cancer types: COAD, LUAD and BRCA. This object can be replaced by the full set of 33 cancer types using 
<<Frequency estimation, echo=TRUE,print=FALSE,eval=FALSE>>= 

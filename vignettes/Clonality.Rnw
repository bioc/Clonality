...
<<Lung and colon frequencies , echo=TRUE,print=TRUE>>= 
  
mut.list<-row.names(freqdata[freqdata[,"COAD"]>=1 | freqdata[,"LUAD"]>=1,][-1,])[1:10000] #restricting set of loci to 10,000 so that the data are more similar to targeted sequencing
freqCOAD<-get.mutation.frequencies(mut.list,"COAD")/freqdata[1,"COAD"] 
freqCOAD[freqCOAD==min(freqCOAD)]<-1/(freqdata[1,"COAD"] +freqdata[1,"LUAD"]  ) #arbitrarily specified background mutation rate
freqLUAD<-get.mutation.frequencies(mut.list,"LUAD")/freqdata[1,"LUAD"]  
freqLUAD[freqLUAD==min(freqLUAD)]<-1/(freqdata[1,"COAD"] +freqdata[1,"LUAD"]  )  #arbitrarily specified background mutation rate
